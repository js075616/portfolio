...
def gains_table(passed_csv):
    ...
    for column in range(1, 1411):
        ...
        if pctl == 0:
            ...
            pctllog = math.log(pctl, 2)
        if pnctl == 0:
            pnctllog = 0
        else:
            pnctllog = math.log(pnctl, 2)
        if pctr == 0:
            pctrlog = 0
        else:
            pctrlog = math.log(pctr, 2)
        if pnctr == 0:
            pnctrlog = 0
        else:
            pnctrlog = math.log(pnctr, 2)
        htl = -1 * ((pctl * pctllog) + (pnctl * pnctllog))
        htr = -1 * ((pctr * pctrlog) + (pnctr * pnctrlog))
        hst = (pl * htl) + (pr * htr)
        gains[column] = ht - hst
    # total_max_index = max_index
    ...
    for i in range(0, 10):
        ...
        table.append(
            [gene_names[max_index], ntl, ntr, ntlc, ntlnc, ntrc,
             ntrnc, str(round(pl * 100,3)) + "%", str(round(pr * 100,3)) + "%", hst, ht, gains[max_index]])
        gene_names.pop(max_index)
        gains.pop(max_index)
    print(tabulate(table, headers='firstrow', tablefmt='fancy_grid', numalign="right"))
    print("")
